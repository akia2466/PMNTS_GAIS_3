...
const DashboardLayout: React.FC<Props> = ({ user, activeTab, setActiveTab, logout, children }) => {
  ...
  return (
    <div className="flex h-screen bg-[#F8F8F8] overflow-hidden font-sans">
      ...
      {/* Mobile Drawer */}
      <div className={`fixed top-0 left-0 bottom-0 w-72 bg-black text-white z-[101] flex flex-col transform transition-transform duration-300 lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gold rounded-lg flex items-center justify-center font-bold text-black text-xl shadow-lg">P</div>
            <h1 className="text-xl font-heading font-extrabold tracking-tighter uppercase">POMNHS</h1>
          </div>
      ...
      <div className="flex-grow flex flex-col relative overflow-hidden">
        {/* Topbar */}
        <header className="h-20 bg-white border-b border-gray-100 px-6 sm:px-10 flex items-center justify-between z-10 shrink-0">
          <div className="flex items-center space-x-4 flex-grow max-w-2xl">
            {/* Mobile Menu Toggle */}
            <button 
              onClick={() => setIsMobileMenuOpen(true)}
              className="lg:hidden p-2 bg-gray-50 rounded-xl text-black hover:bg-gold transition-colors mr-2"
            >
              <Menu size={24} />
            </button>
        ...
        {/* Content Area */}
        <main className="flex-grow overflow-y-auto custom-scrollbar px-4 sm:px-6 py-8 sm:px-10 bg-[#F8F8F8]">
          <div className="max-w-[96rem] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            {children}
          </div>
        </main>
      </div>
